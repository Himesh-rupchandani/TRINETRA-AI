"""Multi-scale vehicle geometry for the standalone scripts.

This file is a VERBATIM MIRROR of the backend's
``TRINETRAAI/backend/app/services/detection_core.py`` (see that module's
docstring for why the rules are what they are). ``trinetra_detection`` is a
self-contained deliverable that must run without the backend on sys.path, so it
carries its own copy - and ``tests/test_multiscale.py`` feeds both modules the
same frames, the same fake model and the same settings and fails if a single
box or pass differs. Read this file, never edit it alone.

The rule it exists to enforce: **resolution, not padding**. A 1280x720 frame
squeezed into imgsz 960 shows every vehicle at 75%; two parked bikes then
overlap enough for NMS to merge them and a 20 px vehicle falls under the
model's floor. Slicing the frame into vertical strips and running each at its
own native resolution fixes both, and the boxes are mapped back by the crop
offset - information no post-processing can recover.

Number plates are never multi-scaled here: that stage keeps its own historical
conf/imgsz so that vehicle quality cannot silently move number-plate recall.
"""
