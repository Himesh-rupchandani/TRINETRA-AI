...
import { EvidenceThumbs } from '@/components/vehicle/VehicleLogGallery';
  ...
  return (
    <>
          ...
          <tbody>
            {events.map((e) => (
              <tr key={e.id} className={cn(activeEventId === e.id && 'bg-high/10')}>
                ...
                <td>
                  <EvidenceThumbs event={e} onClick={() => setPreview(e)} />
                </td>
                <td>
      ...
      <Modal
        ...
        title={preview ? `Evidence — ${preview.plate}` : ''}
        subtitle={preview ? `${preview.cameraName} · ${preview.location}` : undefined}
      >
        {preview && <EvidencePanel event={preview} />}
      </Modal>
    </>
  );
}
